...
public class MySQL5Dialect implements Dialect {
	...
	public final static String SQL_FILED = "SELECT TABLE_CATALOG,TABLE_SCHEMA,COLUMN_NAME,ORDINAL_POSITION,COLUMN_DEFAULT,IS_NULLABLE,DATA_TYPE, "
			+ " CHARACTER_MAXIMUM_LENGTH,NUMERIC_PRECISION,NUMERIC_SCALE,COLUMN_COMMENT "
			+ " FROM INFORMATION_SCHEMA.COLUMNS  " + " WHERE 1=1  AND TABLE_NAME = ? and table_schema =  '"
			+ "DBUtils.DB_NAME" + "'";

	// 1.查询记录数
	public final static String SQL_COUNT = "\t<select id=\"doSearchCount\" parameterType=\"$_voName$\" resultType=\"java.lang.Integer\">\n"
			+ "	\tSELECT COUNT(0) AS _COUNT FROM $TABLE_NAME$ $PREFIX$ WHERE 1=1 AND DELETED_FLAG = '0'\n"
	...
	public static final String SERVICE_IMPL_TEMPLATE = "package $packagePath$.service.impl;\n\n"
			...
			+ "import com.$frameName$.uniframe.platform.dao.BaseDAO;\n"
			+ "import com.$frameName$.uniframe.platform.service.impl.BaseServiceImpl;\n"
			...
			+ "\tpublic BaseDAO<$_voName$> getBaseDAO() {\n" + "\t\treturn $daoName$;\n" + "\t}\n" + "}";
