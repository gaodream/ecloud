...
public class ZKClient {
    ...
    static ExecutorService listenPool = Executors.newCachedThreadPool();
	...
	public static boolean connect(String connectString, int sessionTimeOut, int connectionTimeout) {
		ZKClient.connectString = connectString;
		ZKClient.sessionTimeOut = sessionTimeOut;
		ZKClient.connectionTimeOut = connectionTimeout;
		CuratorFrameworkFactory.Builder builder = CuratorFrameworkFactory.builder();  
	...
	public static boolean connect(String connectString,int sessionTimeOut) {
    ...
    public static NodeCache addNodeListener(String path, NodeCacheListener listener) throws Exception {
    	...
    	return addNodeListener(cache,listener);
    }
    
    protected  static NodeCache addNodeListener(NodeCache nodeCache, NodeCacheListener listener) throws Exception {
    	nodeCache.getListenable().addListener(listener, listenPool);
    ...
    public static TreeCache addTreeListener(String path,TreeCacheListener listener) throws Exception {
        String trimPath = formatPath(path);
        final TreeCache treeCache = new TreeCache(zkClient, trimPath);
	...
	public static void main(String[] args) {
		ZKClient.connect("192.168.6.112:2181");
		try {
			ZKClient.addPathListener("/zookeeper", new PathListener() {
				
				@Override
				public void changeEvent() {
					// System.out.println("event="+JSON.toJSONString());
				}
			});
		} catch (Exception e) {
			e.printStackTrace();
